...
class Triceps : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val recyclerView: RecyclerView = findViewById(R.id.tricepsView)
            val tricepsExerciseList: MutableList<ExcerciseAdd> = mutableListOf()
            ...
            exerciseRef.addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    tricepsExerciseList.clear()
                    for (childSnapshot in snapshot.children) {
                        val exercise = childSnapshot.getValue(ExcerciseAdd::class.java)
                        if (exercise?.muscleGroup == "Triceps") {
                            exercise?.let {
                                tricepsExerciseList.add(it)
                            }
                        }
                    }

                    val adapter = ExerciseAdapter(tricepsExerciseList)
                    recyclerView.adapter = adapter
                    recyclerView.layoutManager = LinearLayoutManager(this@Triceps)
                }
