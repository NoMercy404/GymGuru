...
class Biceps : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val recyclerView: RecyclerView = findViewById(R.id.bicepsView)
            val bicepsExerciseList: MutableList<ExcerciseAdd> = mutableListOf()
            ...
            exerciseRef.addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    bicepsExerciseList.clear()
                    for (childSnapshot in snapshot.children) {
                        val exercise = childSnapshot.getValue(ExcerciseAdd::class.java)
                        if (exercise?.muscleGroup == "Biceps") {
                            exercise?.let {
                                bicepsExerciseList.add(it)
                            }
                        }
                    }

                    val adapter = ExerciseAdapter(bicepsExerciseList)
                    recyclerView.adapter = adapter
                    recyclerView.layoutManager = LinearLayoutManager(this@Biceps)
                }
